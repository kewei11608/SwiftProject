...
class YJTabbarViewController: UITabBarController {
    ...
    var  items:NSArray = []
    let NameArr = ["课程报名","我的报名","我的","练习"]
    let PicArr = ["enroll_blur","baokao_blur","my_blur","my_blur"]
    let PicSelectArr = ["enroll_focus","baokao_focus","my_focus","my_focus"]
    let VCArr = [ApplicationMainController(),MyApplicationMainController(),MyMainViewController(),MyTestTableController()]
    //初始化数组
    ...
    func CreatTabBar()  {
        ...
        let TestVC  = MyTestTableController()
        TestVC.title = "练习"
        let TestNav = YJNavigationController(rootViewController:TestVC)
        TestNav.tabBarItem.title = "练习"
        TestNav.tabBarItem.image = UIImage(named:PicArr[2])
        TestNav.tabBarItem.selectedImage = UIImage(named:PicSelectArr[3])?.withRenderingMode(.alwaysOriginal)

        
        
        
        // 添加工具栏
        items = [AppleicationNav,MyApplicationNav,MyMainNav,TestNav]
