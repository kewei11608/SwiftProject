...
extension YJApplicationService{
    ...
    class func getRequestData (requestUrl:NSString,dict:NSDictionary,finish: @escaping (_ success: Bool,_ model: YJApplicationClassListModel?,_ errorMsg: String?) -> Void){
...
//        let requestUrl : String = ApplicationCommonUrl.appClassList
        ...
        YJNetWorkTool.RequestWithURL(url: requestUrl as String, method: .get, parameter: dict as? [String : AnyObject] ) {
            
            (_ model: YJApplicationClassListModel?, response: YJNetWorkResponse) in
            
//            let jsonString = JSON(response.result.value as Any).rawString()
//
//            let returndata = JSON(parseJSON: jsonString!)
//
