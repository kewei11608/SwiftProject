...
import Foundation
class AFNMoyaController: UIViewController {
...
extension AFNMoyaController: UITableViewDataSource, UITableViewDelegate  {
...
//            }.request()
    ...
    func loadDataByAFN() {
        NetworkTools.POST(url: "http://app.u17.com/v3/appV3_3/ios/phone/rank/list", params: nil, success: { (json) in
            ...
            let model = try? decoder.decode(DMModel.self, from: json)
            self.model = model
            print("\(String(describing: model?.data.returnData))")
    ...
    /// 类AFN请求
    func loadDataByAFNTest() {
        NetworkTools.POST(url: "http://f9v0.cn/api/baseData_gameColumn", params: nil, success: { (json) in
            let decoder = JSONDecoder()
            
            print("\(decoder)")
            let model = try? decoder.decode(AFNModel.self, from: json)
         
            print("\(String(describing: model?.code))")
